...
def _case(captured, alert) -> str:
    ...
    if alert.breaks is not None:
        lines.append(f"BREAKS: {alert.breaks.id} — {alert.breaks.statement} (premise: {alert.breaks.rationale})")
...
def convene(conn, captured, client=None, model: str = MODEL) -> Verdict:
    ...
    if not alerts:
        return Verdict(False, "ALLOW", rationale="No prior premise is broken.")
    alert = alerts[0]
