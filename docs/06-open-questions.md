...
|---|---|---|---|
| A1 | **Name = "Trace"** (chosen by the team, 28 Jun; was provisionally "Keystone"). | Speaks to traceability / the audit trail / provenance — *a trace of every decision and why it was made*; aligns with the decision-record theme; short + demo-memorable. | Settled. Repo + all docs renamed to Trace. |
| A2 | ~~Lead with the UK golden thread / BSA-2022 wedge~~ **SUPERSEDED 2026-07-10: lead with the Singapore QP-liability wedge (Building Control Act s.9 + the reg 22(e) gap); UK erased from all pitch surfaces.** | The team knows the Singapore regime first-hand and cannot defend UK-code questions in Q&A; all-Singapore also reads sharper to APAC judges (three SG projects, two SG code authorities). | Settled — executed across README, docs 00/01/02/03/05/10, demo script, scenarios and rule-packs. |
...
| A5 | **Demo storyline = "Tanglin Rise" facade (mineral rainscreen → PE-core ACP) under SCDF Cl 3.5.1.** | Highest blast-radius, photogenic red alert, maps 1:1 to the s.9 liability wedge. **The original UK "Maple Wharf" storyline was fully removed on 2026-07-10, replaced by "Pearl Vista" (BCA Periodic Façade Inspection — the second Singapore code authority).** The other 3 worked examples (parking, floor-to-floor, core) are backups. | Swap to parking/floor-to-floor/core if you prefer (all in [01](01-aec-direction.md) §4). |
...
2. **Region/jurisdiction focus** — ✅ *Resolved: you're in Singapore.* See **[07-singapore-angle.md](07-singapore-angle.md)** — Singapore has no golden thread, but the **QP personal-liability** regime is a stronger home-market wedge. Recommended framing: Singapore → China → UK (one line). **✅ Now locked (30 Jun):** fully localize the demo to Singapore — **"Tanglin Rise"** (SCDF Fire Code 2023 Cl 3.5 non-combustible **> 15 m**; **Building Control Act s.9** QP personal criminal liability; **Toh Guan Road** fire precedent), and — decided 2026-07-10 — **the UK is erased from the pitch entirely** (the team cannot defend UK-code Q&A; all-Singapore is firmer). The doc rewrites from 07 §6 are executed.
3. **Should I scaffold the code now?** I can stand up the repo: schema + SQLite store + the `capture_decision` / `check_invalidation` Qwen-Agent tools + the demo transcripts (Tanglin Rise since 30 Jun) + a thin CLI, so you have a running spine on day one. Say the word.
