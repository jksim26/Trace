# 07 · The Singapore Angle — Localizing the Pitch

*Research pass run 2026-06-28 for a Singapore team facing Alibaba/Qwen (China/APAC) judges. Adversarially fact-checked against primary sources (Building Control Act, SCDF Fire Code, URA circulars, BCA media releases). Confidence tags: `[verified]` (primary source read) · `[web search]` · `[inference]`.*

---

## 0. The honest bottom line (read this first)

**Singapore has no "golden thread."** No Singapore statute or regulator (BCA, SCDF, SSO) mandates a single attributable, immutable, lifecycle design-decision/safety-information record. The UK Building Safety Act 2022 golden thread (HRBs ≥18 m / 7 storeys, in force 1 Oct 2023) has **no Singapore counterpart.** `[verified — absence confirmed across BCA/SCDF/SSO]`
...
## 1. The QP regime — our Singapore wedge

**Who is a QP.** A *Qualified Person* is a **registered Architect** (Architects Act 1991; Board of Architects / BOA) or **registered Professional Engineer** (Professional Engineers Act 1991; Professional Engineers Board / PEB) holding a **current annual practising certificate.** Plans can only be submitted to BCA through a *named* QP. `[verified — BOA/PEB primary]` *(Don't put "PE Act s.18" on a slide — the section number is unverified.)*
...
1. **structural-only** (nothing on fire, MEP, façade rationale),
2. about **deviations from approved plans**, not the **rationale/assumptions** behind decisions,
...
With a China-HQ'd Alibaba/Qwen judge panel, a UK-only frame reads as a foreign-compliance niche. Sequence it: `[inference — strategy]`
...
## 4. Localized demo variant — "Tanglin Rise" (Singapore)
...
## 5. Verified facts — and what you may NOT put on a slide

### ✅ Safe to use (high confidence, primary-sourced)
| Fact | Note |
|---|---|
| No SG golden thread; QP personal criminal liability is the hook | frame as *absence*, not "rejected" |
| Building Control Act **s.9** personal due-diligence duty (verbatim) | the core legal anchor |
| Building Control Regs **reg 22(e)** structural-deviation record = only analogue, no falsification detection | the gap = our wedge |
| QP = registered Architect (Architects Act 1991/BOA) or PE (PE Act 1991/PEB) + annual practising cert | **omit the "s.18" number** |
| **CORENET X** mandatory 1 Oct 2025 / 2026 / 2027; OpenBIM (IFC) | URA circular DC25-07 |
| **IDD target 70% by 2025** (from 34%); DfMA 70% by 2025 | use the *target*, not the 58/67% progress |
| **SCDF Fire Code 2023**: non-combustible external wall **> 15 m** (Cl 3.5); ACP outer layers Class 0 (Cl 3.15) | **use 15 m** |
| **Toh Guan Road fire** 4 May 2017, 1 fatality; SCDF found **40 buildings** | **use 40**; preceded Grenfell |
| **PFI**: effective 1 Jan 2022; > 13 m & > 20 yrs; every 7 yrs; ~30,000 buildings | enabling Act passed 6 Mar 2020 |
| **Fire Safety (Amendment) Act 2019** supply-chain liability; in force 14 Sep 2020 | passage date 5 Aug 2019 = medium |
| Building Control (Amendment) Act **2024 = energy efficiency only** | do **not** miscite as accountability reform |
| China "lifelong quality responsibility" principle | cite the *principle*, not the order number |

### ⛔ Do NOT slide without first verifying against a primary source
- "PE Act **s.18**" (section number) · s.9 **penalty ceilings** (S$200k/S$100k) · **MCST Plan 1075 v RSP Architects** case citation · China **MOHURD order number** · IDD **58%/67%** progress figures · Fire Safety Amendment **passage date** · 22 May 2026 developer-framework mechanics · **378,000** foreign construction workers (re-confirm MOM). **No statutory document-retention period exists — don't assert one.**

---

## 6. Concrete doc changes (execute once you lock framing — decision #2 in [06](06-open-questions.md))

> I have **not** rewritten the other docs yet — the framing is your call with your teammate. When you decide, here's the edit list:

- **[00-brief.md](00-brief.md) §4** — add a Singapore-first paragraph (QP personal liability + reg 22(e) gap); demote UK golden thread to a one-line precedent.
- **[01-aec-direction.md](01-aec-direction.md) §2** — add a Singapore subsection (the absence finding + the QP/reg 22(e) wedge); **§4** — add the "Tanglin Rise" Singapore façade variant alongside the existing examples.
- **[03-hackathon-strategy.md](03-hackathon-strategy.md) §3–4** — re-lead the Problem-Value framing with Singapore + China; demote UK to one line.
- **demo** — re-author "Maple Wharf" → "Tanglin Rise" (SCDF Cl 3.5 + s.9 + Toh Guan, not ADB B4 + Gateway 2), or keep both as UK/SG variants.
- **all docs** — when you cite numbers: IDD **70% by 2025**, cladding count **40**, height threshold **15 m**, and flag every ⛔ item above.

---

## 7. Open items to close before the pitch

- Verify the s.9 **penalty ceilings**, **PE Act practising-cert section**, and the **MCST Plan 1075** citation on SSO.
- Confirm whether **CORENET X already provides a submission audit trail** (competitive overlap).
- Re-confirm the **378,000** MOM construction-workforce figure and the **22 May 2026** developer-framework mechanics against BCA primary notices.
- Decide: localize the demo fully to Singapore, or run **UK + SG variants** (UK = "a government legislated it"; SG = "your QP is personally liable").

---

*Raw Singapore research + full verification: [.research/singapore/](../.research/singapore/). This doc supersedes nothing yet — it's the input to your framing decision.*
