...
4. **How it's built.** Bi-temporal decision store (never-delete supersede) → deterministic rule-pack gate → retrieve-to-budget. Qwen stack: qwen-plus extraction, the decision court's three Qwen roles, custom tools via Qwen-Agent, Singapore endpoint. Note: 52 tests, deterministic alert gate = reliability by design.
